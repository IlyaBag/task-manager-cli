...
def test_get_storage_state(create_test_storage, create_test_tasks):
    task1, task2 = create_test_tasks
    expected_state = StorageState(id_count=123, tasks=[task1, task2])
...
def test_save_storage_state(create_test_storage, create_test_tasks):
    ...
    new_state = StorageState(id_count=23, tasks=[task1, task2])

    path = create_test_storage
    utils.save_storage_state(path, new_state)

    expected_data = (
        '0023id,title,description,category,due_date,priority,status\n'
...
def test_get_id_count(create_test_storage):
    expected_id_count = 123
    id_count = utils.get_id_count(create_test_storage)
